...
QSPIDevice::QSPIDevice(int bus, int channel, QObject *parent) :
    ...
    d_ptr(new QSPIDevicePrivate())
{
    QString strFileName = QStringLiteral("/dev/spidev%1.%2").arg(bus).arg(channel);
...
bool QSPIDevice::open(OpenMode flags)
{
    ...
    if(QSPIDevicePrivate::remoteServerIP.isEmpty()) {
        ...
        QStringList	strList = d->devFileName.split(QStringLiteral("/"));
...
bool QSPIDevice::setLSBFirst(bool lsbFirst)
{
    ...
    if(d->remoteClient == nullptr) {
        ...
        if(!isOpen()) {
            ...
            __u8 lsb = static_cast<__u8>(lsbFirst);
...
bool QSPIDevice::setBitsPerWord(quint8 bitsPerWord)
{
    ...
    if(d->remoteClient == nullptr) {
        ...
        if(!isOpen()) {
            ...
            __u8 bits = static_cast<__u8>(bitsPerWord);
...
bool QSPIDevice::setBitSpeed(quint32 bitSpeedHz)
{
    ...
    if(d->remoteClient == nullptr) {
        ...
        if(!isOpen()) {
            ...
            __u32 hz = static_cast<__u32>(bitSpeedHz);
...
bool QSPIDevice::sendReceive(QByteArray &dataSend, QByteArray &dataReceive)
{
    ...
    if(d->remoteClient == nullptr) {
        ...
        if(bOK) {
            ...
            spi_io.tx_buf = reinterpret_cast<__u64>(dataSend.constData());
            spi_io.rx_buf = reinterpret_cast<__u64>(dataReceive.data());
            spi_io.len = static_cast<__u32>(dataSend.size());
...
static quint8 reverse(quint8 b) {
   b = static_cast<quint8>((b & 0xF0) >> 4 | (b & 0x0F) << 4);
   b = static_cast<quint8>((b & 0xCC) >> 2 | (b & 0x33) << 2);
   b = static_cast<quint8>((b & 0xAA) >> 1 | (b & 0x55) << 1);
   return b;
}
    ...
    if(d->remoteClient == nullptr) {
        ...
        if(d->bSWReverseRequired) {
            for(qint64 iByte=0; iByte<bytesRead; iByte++) {
                data[iByte] = static_cast<char>(reverse(static_cast<quint8>(data[iByte])));
            }
        }
    }
    else {
...
qint64 QSPIDevice::writeData(const char *data, qint64 len)
{
    ...
    if(d->remoteClient == nullptr) {
        if(d->bSWReverseRequired) {
            char *copyData = new char[static_cast<quint32>(len)];
            if(copyData) {
                for(qint64 iByte=0; iByte<len; iByte++) {
                    copyData[iByte] = static_cast<char>(reverse(static_cast<quint8>(data[iByte])));
                }
